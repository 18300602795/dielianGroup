...
import com.etsdk.app.huov7.chat.pickerimage.utils.AttachmentStore;
import com.etsdk.app.huov7.chat.pickerimage.utils.StorageUtil;
import com.etsdk.app.huov7.chat.utils.BitmapLoader;
import com.etsdk.app.huov7.chat.utils.DialogCreator;
import com.etsdk.app.huov7.chat.utils.NativeImageLoader;
import com.etsdk.app.huov7.chat.view.ImgBrowserViewPager;
import com.etsdk.app.huov7.chat.view.PhotoView;
...
//用于浏览图片
public class BrowserViewPagerActivity extends ImmerseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(AileApplication.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(AileApplication.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(AileApplication.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(AileApplication.POSITION, 0);
...
//                                AileApplication.forwardMsg.add(mMsg);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(AileApplication.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(AileApplication.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(AileApplication.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(AileApplication.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(AileApplication.MsgIDs, activity.mMsgIds);
                        activity.setResult(AileApplication.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
